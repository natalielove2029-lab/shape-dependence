...
void lastminute(ofstream *fneu)
	...
	for (std::map<int, node_type>::iterator iter=nodes.begin(); iter!=nodes.end(); iter++)
		{x=iter->first;
	if (kiiratas>=4) {entropiakiiras(x,fneu);}
...
void hinsert(int itt, ofstream *fneuq)
	...
	nodes[itt].gap=-1;
	if (kiiratas>=4) {entropiakiiras(itt,fneuq);}
...
void ch(int p, int q)
    ...
    //he letezik az el
    if (nodes[p].edges.find(p+q)!=nodes[p].edges.end())
    {
...
int kezdi=(int) time(NULL);
	double szoras=0,varhato=0, vperk=0, vw=0;
...
//cout <<"R"<<realizacio <<"\11";
            ...
            //
                ...
                for(i=0; i<Nodes; i++)
                    ...
                    if (mtrand.rand()<0.5)
                    {
                    if (div(i,meret).rem==(meret-1))
                        {//atkotjuk a bal szelevel
                        in(i,-(meret-1));
                        nodes[i].gap=-3;
                        midway++;
                        nodes[i-(meret-1)].gap=-3;
                        midway++;
                        }
                    else
                        {//jobbra kotjuk
                ...
                for(i=0; i<Nodes; i++)
                    ...
                    if (div(i,meret).quot==(meret-1))
                        ...
                        {midway++;
                            ...
                            nodes[i].gap=-3;
                            if (nodes[i-(meret-1)*meret].gap!=-3)
                            {midway++;
                            }
                        nodes[i-(meret-1)*meret].gap=-3;
		...
		lsta << setprecision(12) << varhato << '\11' << szoras << '\11'<< vperk  << '\11'<< vw << '\11'<< rep << '\11' << hmaxmax<< '\n';
		meret*=2;
		}
